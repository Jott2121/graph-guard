...
- **Headline result is NEGATIVE and that is deliberate:** on 82 real sessions over a 586-note corpus the graph did **not** beat plain chunk-level TF-IDF (flat 0.6904 vs ensemble+PPR 0.6537 recall@20, n.s.). The old +14%/+26% headline came from **structure-derived probes** (queries synthesized from the graph's own edges = circular) and is kept only for contrast. Honest eval = `eval/real_sessions_ab.py`; do not restore the circular one as the headline.
